...
if echo "$PR_BODY" | grep -q "Ticket:"; then
  ...
  if ! echo "$PR_BODY" | tr -d '\r' | grep -q "^Ticket: \(ST\|NT\)-[0-9]*$"; then
    echo "Error: Ticket format is invalid. Should be 'Ticket: ST-XXXX' or 'Ticket: NT-XXXX' on its own line where XXXX are numbers."
    exit 1
  fi
fi
