...
class CompilerServiceTest : FunSpec({
    val waitTimeMs = 500L
            ...
            val scenario = TestScenario(
                ...
                expectedDiagnostics = listOf(
                    ExpectedDiagnostic(
                        ...
                        code = 89,
                        message = "The compiler cannot (currently) automatically resolve the type for '/test/doIt'. Please add explicit types.",
                        severity = DiagnosticSeverity.Error,
                    ),
                ),
            )
...
private fun withTestService(expectedDiagnosticsCount: Int = 1, test: (CompilerService, TestLanguageClient) -> Unit) {
